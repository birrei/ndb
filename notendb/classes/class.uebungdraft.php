...
class Uebung {
  ...
  public $Title='Übung Entwurf';
  public $Titles='Übungsentwürfe';  
  public $table_name='uebungdraft'; 
  ...
  function insert_row (string $SchuelerID, string $Datum) {
    ...
    $insert = $this->db->prepare("INSERT INTO `uebungdraft` 
  ...
  function load_row() {  
    ...
    $select = $this->db->prepare("SELECT 
                              uebungdraft.ID
                            , uebungdraft.Name
                            , uebungdrafttyp.Name as Typ
                            , COALESCE(uebungdraft.Bemerkung, '') as Bemerkung
                            , uebungdraft.uebungdrafttypID
                            , uebungdraft.SchuelerID
                            , uebungdraft.Anzahl 
                            , uebungdraft.SatzID
                            , schueler.Name as SchuelerName
                            , COALESCE(bewertung.Name, '') as Bewertung  
                          FROM  uebungdraft 
                                left join uebungdrafttyp on uebungdraft.uebungdrafttypID = uebungdrafttyp.ID 
                                left join schueler on uebungdraft.SchuelerID = schueler.ID 
                                left join bewertung on bewertung.ID = uebungdraft.BewertungID 
                          WHERE uebungdraft.ID = :ID");
    ...
    if ($select->rowCount()==1) {
      ...
      $this->uebungdrafttypID=$row_data["UebungtypID"];      
  ...
  function load_row_Datum() {  
    ...
    $select = $this->db->prepare("SELECT uebungdraft.Datum
                          FROM  uebungdraft 
                          WHERE uebungdraft.ID = :ID");
  ...
  function update_row ( $SchuelerID  
    ...
    $update = $this->db->prepare("UPDATE uebungdraft  
  ...
  function delete(){
    ...
    $delete = $this->db->prepare("DELETE FROM `uebungdraft` WHERE ID=:ID"); 
  ...
  function copy(){
    ...
    $sql="INSERT INTO uebungdraft (Name
                            , Bemerkung
                            , UebungtypID
                            , SchuelerID
                            , Datum
  ...
  function print_table_lookups($target_file, $LookupTypeID=0){
    $query="SELECT lookup.ID
             ...
             , lookup.Name  
          FROM uebungdraft_lookup          
          INNER JOIN lookup 
            on lookup.ID=uebungdraft_lookup.LookupID
          INNER JOIN lookup_type
            on lookup_type.ID = lookup.LookupTypeID
          WHERE uebungdraft_lookup.UebungID = :UebungID";
  ...
  function add_lookup($LookupID){
    ...
    $insert = $this->db->prepare("INSERT IGNORE INTO `uebungdraft_lookup` SET
        `UebungID`     = :UebungID,  
        `LookupID`     = :LookupID");
  ...
  function delete_lookup($ID){
    ...
    $delete = $this->db->prepare("DELETE FROM `uebungdraft_lookup` 
  ...
  function copy_lookups($ID_new) {
    ...
    $sql="insert into uebungdraft_lookup
          (UebungID, LookupID) 
  ...
  function print_table_satz_lookups_checklist(){
    ...
    $query="select lookup.ID
          ...
          INNER JOIN uebungdraft on uebungdraft.SatzID=satz_lookup.SatzID 
          LEFT JOIN uebungdraft_lookup on uebungdraft_lookup.UebungID = uebungdraft.ID 
              AND uebungdraft_lookup.LookupID = satz_lookup.LookupID 
          WHERE uebungdraft.ID =  :ID
          AND uebungdraft_lookup.ID IS NULL 
          order by Name"; 
  ...
  function get_Order ($Datum) {
    ...
    $this->load_row(); 
    $sql="SELECT (coalesce(MAX(Reihenfolge),0)) + 1 as Reihenfolge_Neu from `uebungdraft` 
