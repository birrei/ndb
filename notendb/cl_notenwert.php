...
class Notenwert {
  ...
  function insert_row ($Name) {
    ...
    $insert = $db->prepare("INSERT INTO `notenwert` 
              SET `Name`     = :Name"
           );
  ...
  function print_select($value_selected=''){
    ...
    try {
      ...
      $html->print_select("NotenwertID", $value_selected, true); 
  ...
  function update_row($Name) {
    ...
    $update = $db->prepare("UPDATE `notenwert` 
                            SET
                            `Name`     = :Name
                            WHERE `ID` = :ID"); 
  ...
  function print_select_multi($options_selected=[]){
    ...
    try {
      $stmt->execute(); 
      $html = new HtmlSelect($stmt); 
      $html->print_select_multi('Notenwert', 'Notenwerten[]', $options_selected); 
    }
    catch (PDOException $e) {
      include_once("ctl_html_info.php"); 
      $info = new HtmlInfo();      
